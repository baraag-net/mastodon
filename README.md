...
- **PostgreSQL full-text status search fallback.** When Elasticsearch is disabled, status search can fall back to PostgreSQL full-text search. This is controlled by `DB_SEARCH_ENABLED`, which defaults to `true`.
